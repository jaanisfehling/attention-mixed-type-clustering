...
class FTTransformerAutoencoder(torch.nn.Module):
    def __init__(self, encoder: nn.Sequential, decoder: nn.Sequential, cont_dim: int,
        ...
        self.cont_embeddings = nn.ModuleList([nn.Linear(1, emb_dim - (emb_dim // 8)) for _ in range(cont_dim)])
        self.shared_embedding = nn.Parameter(torch.empty(len(embedding_sizes) + cont_dim, emb_dim // 8).uniform_(-1, 1))
    ...
    def encode(self, x_cat: torch.Tensor, x_cont: torch.Tensor) -> torch.Tensor:
        ...
        if x_cont.size()[1] > 0:
            x_cont = torch.stack([l(x_cont[:, i].reshape(-1, 1)) for i, l in enumerate(self.cont_embeddings)], 1)
            x = torch.cat((x_cat, x_cont), 1)
        else:
            x = x_cat

        # stretch shared embedding to batch size
        stretched_shared_embedding = self.shared_embedding.unsqueeze(0).repeat(x.size()[0], 1, 1)
        
        # add the shared embedding to embedding dimension
        x = torch.cat((x, stretched_shared_embedding), 2)

        self.last_target = x.flatten(start_dim=1).clone().detach()
