...
We then encode the classes of each categorial feature with integer values, going from 0 onwards. The integer values serve as indices for the correponding weight tensor of our embedding layer. Passing a value $j$ of a feature $i$ in the embedding layer returns the one-dimensional tensor at index $j$ of the weight tensor $e_j$, which is of dimension $1 \times s_i$. The resulting tensors of the embedding layer are concatenated into a tensor of size $1 \times \sum^N_{i=1}s_i$, which includes the embeddings of all categorial features. This tensor will then be concatenated with the tensor of the continuous features, before being passed into the first linear layer of the encoder block.

\section{Attention}

\textit{Attention} is a machine learning mechanism for sequence processing, originally used in machine translation by Bahdanau et al. \cite{bahdanau} and extended into the \textit{Transformer} architecture by Vaswani et al. \cite{attention_is_all_you_need}.
