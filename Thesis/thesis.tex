...
where $w_{ji}^{(n)}$ is the learnable weight of the input $i$ going through neuron $j$ in layer $n$. A bias $b_j$ is also added. This output is commonly refered to as an \textit{activation} of a neuron. The activation $a_j$ is then passed into a activation function $f$
