...
Clustering mixed-type datasets is a difficult task, and many proposed methods do not manage to beat baseline clustering models that were initially designed for continuous data applications. With autoencoder neural networks gaining popularity, the possibility of using autoencoders in clustering tasks has emerged. The autoencoder can learn a dense representation of the data, which can then be used for the following clustering step, where a classical clustering method such as k-means is used. We extend the autoencoder with an attention mechanism, which allows the autoencoder to learn dependencies between features. Further, we extend the attention mechanism into a full Transformer, which is able to provide contextual feature embeddings. We implement a Transformer only for categorical features and a Transformer for both categorical and continuous features. We show that these methods can increase clustering performance on some datasets. We finally combine our models with deep clustering methods DEC, IDEC and DCN, which aim to create dense representations of the input data more feasible for clustering.
...
The layer sizes of the encoder are created by always reducing the tensor size by $t_e$ in each linear transformation ($h_d - 1$ times in total), where $t_e$ is calculated as
...
The decoder layer sizes are then calculated by reducing the layer size by $t_d$ from the output layer in each linear transformation (again, $h_d - 1$ times in total), but inverting the resulting array of layer sizes. This gives us a natural, symmetrical autoencoder structure for each test case (such as Figure \ref{autoencoder_illustration}), which is also used for following experiments. We use mean squared error loss (Section \ref{Neural Networks}) and the Adam optimizer \cite{adam_optimizer} when training any neural network. The results do not clearly indicate which autoencoder architecture is the best. Therefore we build upon prior research on mixed-type classification with attention \cite{tab_transformer, ft_transformer}, where a feed-forward network that includes categorical features as well as continuous features is used. This means we choose the autoencoder that includes both categorical and continuous features.
...
At last, we use the models from the previous comparison, but use the deep clustering techniques DEC, IDEC and DCN (Section \ref{Deep Clustering}). While the accuracy results (Table \ref{deep_comparison_acc}) are good, the Normalized Mutual Information results of these methods (Table \ref{deep_comparison_nmi}) do not indicate that deep clustering methods are particular useful for clustering mixed-type tabular data.
...
\end{center}
\caption{Best Normalized Mutual Information scores (in parenthesis) across all clustering methods. Classical methods are colored in green, methods using an autoencoder with k-means are colored cyan, methods using an autoencoder and attention or a transformer mechanism are colored yellow and methods using DEC, IDEC or DCN are colored magenta.}
\label{final_comparison_nmi}
\end{table}

We also provide an overview that lists the three best clustering methods we used in this work on each of the eight mixed-type datasets in Table \ref{final_comparison_nmi}. The results show that although neural network augmented clustering methods show strong results on some datasets, classic k-means and k-means with one hot encoding only perform worse by a small margin and win out on the other datasets. With the exception of the Auction Verification dataset that contains only one continuous feature, either k-means, k-means with one-hot encoding or both are in the top three performing methods. 12 out of 24 best performing clustering methods across all datasets are either k-means or k-means one-hot.
	
\chapter{Conclusion}

We have successfully implemented self-attention into a autoencoder-k-means clustering model. Further, we extended the attention module into a full Transformer. We also implemented a Transformer that additionally incorporates continuous features. We conducted tests on eight mixed-type datasets and show that such attention networks can improve clustering performance on some datasets. Finally, we test our networks with deep clustering methods DEC, IDEC and DCN. 

However, clustering mixed-type datasets remains challenging. As explained in detail in the previous chapter, in general it is better to rely on k-means or k-means with one-hot encoding when clustering mixed-type datasets in practice. Even on datasets where our proposed models perform better, k-means often does not perform significantly worse. Additionally, training an autoencoder neural network requires more computational resources. Especially considering the complex networks of our multi-layer transformers, large real-world datasets cannot be clustered without the help of graphics processing units (GPUs), tensor processing units (TPUs) or other hardware accelerators.

For further research, methods could be explored that can estimate if an attention based clustering method is effective on a given dataset before clustering. Also, current deep clustering methods focus on text and image data. It could be explored why these methods perform bad with tabular data and if any improvements can be made.