...
At last, we use the models from the previous comparison, but use the deep clustering techniques DEC, IDEC and DCN (Section \ref{Deep Clustering}). While the accuracy results (Figure \ref{deep_comparison_acc}) are good, the Normalized Mutual Information results of these methods (Figure \ref{deep_comparison_nmi}) do not indicate that deep clustering methods are particular useful for clustering mixed-type tabular data.
